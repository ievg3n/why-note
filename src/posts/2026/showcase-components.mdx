...
<Callout type="note">
    **Did you know?** The latest version of Astro allows you to use standard
    `markdown` alongside `mdx` files seamlessly within your collections!
</Callout>
...
<Callout type="warning" title="Breaking Change">
    I recently migrated from Astro v3 to v5. Please ensure your
    `content.config.ts` has been correctly updated to use `astro:content`.
</Callout>
...
<Tabs tabs={["npm", "pnpm", "yarn"]}>
    <TabItem>```bash npm install @lucide/astro ```</TabItem>
    <TabItem>```bash pnpm add @lucide/astro ```</TabItem>
    <TabItem>```bash yarn add @lucide/astro ```</TabItem>
</Tabs>
...
<Steps>
    <div>
        ### Step 1: Initialize your project Run `npm create astro@latest` and
        choose the blog template to instantly get a highly optimized site
        scaffolding.
    </div>
    <div>
        ### Step 2: Add Tailwind CSS Run `npx astro add tailwind` to securely
        install the latest version of Tailwind into your UI layer.
    </div>
    <div>
        ### Step 3: Write MDX Add the `@astrojs/mdx` plugin and start writing
        content instantly!
    </div>
</Steps>
...
<Quote author="Chris Coyier" title="Co-founder of CodePen">
    MDX is a format that lets you seamlessly write JSX in your Markdown
    documents. You can import components, such as interactive charts or alerts,
    and embed them within your content.
</Quote>
...
<LinkCard
    ...
    description="Astro 5 is here and brings a new Content Layer API, improved type safety, and better build performance."
    domain="astro.build"
/>
